...
int main()
{
	...
	double angle;
	cout << "Input rotation angle(in degree): "; cin >> angle;
	angle = PI/180.0*angle;  // convert degree to angle

	Matrix s(3, 3);
	s.matrix[0][0] = cos(angle);
	s.matrix[1][1] = cos(angle);
	s.matrix[1][0] = sin(angle);
	s.matrix[0][1] = -sin(angle);
	s.matrix[2][2] = 1;
